...
export const changePassword = async(req, res)=>{
    ...
    try{
        ...
        if(!isMatch){
            return res.status(400).json({ errors: [{ field: "oldPassword", message: "Old password is incorrect" }] });
        }

        if(oldPassword === newPassword){
            return res.status(400).json({ errors: [{field: 'newPassword', message: 'New password cannot be the same as old password'}]})
        }
