...
function ProductDetailPage() {
    ...
    const handleWishlistToggle = async () => {
        try {
            if (!isInWishlist) {
                await dispatch(
                    addToWishlist({ productId: productDetail._id, selectedSize })
                ).unwrap();
                    ...
                    <Row>
                        ...
                        <Col md={6} className="d-flex">
                            <div className="d-flex flex-column me-3 product-thumbs">
                                {productDetail?.images?.slice(0, 3).map((thumb, i) => (
                                    <Image
                                        key={i}
                                        src={thumb}
                                        thumbnail
                        ...
                        <Col md={6}>
                            ...
                            <div className="mb-3">
                                ...
                                {productDetail?.sizes.map((s, index) => (
                                    <Button
                                        ...
                                        onClick={() =>
                                            s.stock > 0 && setSelectedSize(s.size)
                                        }
                                        disabled={s.stock === 0}
                                        style={{
                                            textDecoration:
                                                s.stock === 0 ? "line-through" : "none",
                                        }}
                                    >
                                        {s.size.length <= 3
                                            ? s.size.toUpperCase()
                            ...
                            <div className="d-flex align-items-center mb-3">
                                {productDetail?.sizes.some((s) => s.stock > 0) ? (
                                    <>
                            ...
                            {productDetail?.sizes
                                .filter(
                                    (s) => s.size === selectedSize && s.stock <= 2 && s.stock > 0
                                )
                                .map((s) => (
                            ...
                            {/* Tabs */}
                            <Tabs defaultActiveKey="details" id="product-tabs" className="mt-4">
                                <Tab eventKey="details" title="Details">
                                    <div className="pt-3">
                                        <p>
                                            Soft cotton-feel top sheet, contoured wings, and absorbent
                                            multi-layer core for maximum protection.
                                        </p>

                                        {/* Accordion Section */}
                                        <Accordion alwaysOpen={false} flush>
                                            <Accordion.Item eventKey="0">
                                                <Accordion.Header>Selling Partner</Accordion.Header>
                                                <Accordion.Body>Flowell Official</Accordion.Body>
                                            </Accordion.Item>

                                            <Accordion.Item eventKey="1">
                                                <Accordion.Header>Terms & Conditions</Accordion.Header>
                                                <Accordion.Body>
                                                    Non-returnable item due to hygiene concerns.
                                                </Accordion.Body>
                                            </Accordion.Item>
                                            ...
                                            <Accordion.Item eventKey="2">
                                                ...
                                                <Accordion.Body>
                                                    Change pad every 4-6 hours. Wrap used pad and dispose in
                                                    a bin. Do not flush.
                                                </Accordion.Body>
                                            </Accordion.Item>
                                        </Accordion>
                                ...
                                {similarProducts.map((product, i) => (
                                    <Col xs={6} lg={3} key={i}>
                                        ...
                                        >
                                            ...
                                            />
                                            <div className="similar-name">{product.name}</div>
                                            <div className="similar-size">
                                                Size:{" "}
                                                {product.sizes.map((s, index) => (
                                                    <span key={index} className="similar-size-btn">
                                                        {s.size.length <= 3
                                                            ? s.size.toUpperCase()
                                                            : s.size.charAt(0).toUpperCase()}
                                                    </span>
