...
export const getCategory = createAsyncThunk('/category/getCategory', async({page = 1, limit = 10, search = ''}, {rejectWithValue})=>{
    try{
...
export const getSingleCategory = createAsyncThunk('/category/getSingleCategory', async(id, {rejectWithValue})=>{
    try{
...
export const editCategory = createAsyncThunk('/category/editCategory', async({id, formData}, {rejectWithValue})=>{
    try{
        const response = await instance.patch(`/admin/category/${id}`, formData);
        return response.data.category;
    }catch(err){
        return rejectWithValue(err.response.data.errors);
    }
...
export const changeStatus = createAsyncThunk('/category/changeStatus', async(id, {rejectWithValue})=>{
    try{
        const response = await instance.patch(`admin/category/status/${id}`);
        return response.data.category;
    }catch(err){
        return rejectWithValue(err.response.data);
    }
...
const categorySlice = createSlice({
    ...
    extraReducers: (builder)=>{
        ...
        .addCase(editCategory.fulfilled, (state, action)=>{
            ...
            state.errorByAction.editCategory = null;
            const index = state.category.findIndex(category => category._id === action.payload._id);
            if(index !== -1){
