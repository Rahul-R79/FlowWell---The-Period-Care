...
export const addMoneyToWallet = createAsyncThunk(
    "/user/addMoneyToWallet",
    async ({ addAmount }, { rejectWithValue }) => {
        try {
            const response = await instance.post("/user/wallet/add", {
                addAmount,
            });
...
export const verifyWalletPayment = createAsyncThunk(
    "/user/wallet/verifyWalletPayment",
    async (
        {
            userId,
            razorpay_order_id,
        ...
        try {
            const response = await instance.post("/user/wallet/add/verify", {
                userId,
                razorpay_order_id,
                razorpay_payment_id,
                razorpay_signature,
    ...
    async ({ page = 1, limit = 5 }, { rejectWithValue }) => {
        try {
            const response = await instance.get(
                `/user/wallet/transaction?page=${page}&limit=${limit}`
            );
            return response.data;
        } catch (err) {
            return rejectWithValue(err.response.data);
