...
export const verifyForgotOTP = createAsyncThunk(
    ...
    async ({ email, otp }, { rejectWithValue }) => {
        try {
            const response = await instance.post("auth/forgot-verify", {
                email,
                otp,
            });
            return response.data;
...
export const resendForgotOTP = createAsyncThunk(
    ...
    async ({ email }, { rejectWithValue }) => {
        try {
            const response = await instance.post("auth/forgot-resend", {
                email,
            });
            return response.data;
        } catch (err) {
...
export const resetForgotPassword = createAsyncThunk(
    ...
    async ({ email, formData }, { rejectWithValue }) => {
        try {
            const response = await instance.post("auth/reset-forgot-password", {
                email,
                ...formData,
            });
            return response.data;
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteAccount.pending, (state) => {
                state.loadingByAction.deleteAccount = true;
                state.errorByAction.deleteAccount = null;
            })
            .addCase(deleteAccount.fulfilled, (state, action) => {
                state.loadingByAction.deleteAccount = false;
                state.errorByAction.deleteAccount = null;
                state.user = null;
            })
            .addCase(deleteAccount.rejected, (state, action) => {
                state.loadingByAction.deleteAccount = false;
                state.errorByAction.deleteAccount = action.payload;
            });
    },
});

export const { clearErrors, setCurrentUser } = authUserSlice.actions;
